...
in 2D and simulate them 3D\footnote{This section only describes the
  ...
  \ref{interface-implementation}, \nameref{interface-implementation}, on
...
feature that contains it's driving fold\footnote{The exception to this
  rule is holes ---~a hole's parent is the plane that contains it.}.
These parent-child relationships are described in more detail in the
section \nameref{hierarchy} on page \pageref{hierarchy} and in
\citet{mallen}.
...
subclass of the base FoldFeature class.
...
\nameref{constraints-on-fold-features} on page
